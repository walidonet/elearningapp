...
`                }
    ...
    getFullCourse(course:any,force:boolean=false){
        ...
        if(Array.isArray(allc) && allc.indexOf(course.id) != -1 && !force){
            ...
            if(this.config.isLoggedIn){
                ...
                this.fullcourseobservable =  this.http.get(`${this.config.baseUrl}course/`+course.id,opt).map(response =>{
    ...
    */
    ...
    checkAndAssignPmproLevel(pricing,fullcourse){
        ...
        let opt = this.auth.getUserAuthorizationHeaders();
        return this.pmprochecklevelobservable =  this.http.get(`${this.config.baseUrl}user/course/pmprochecklevel/`+fullcourse.course.id+`/level/`+pricing.id,opt).map(response =>{
